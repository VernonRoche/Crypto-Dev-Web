<template>
    ...
    <div v-if="ischangedPassword">
        <input type="text" placeholder="enter the new PassWord" v-model="newPassword"/>
        <br/>
...
import { getAuth, onAuthStateChanged , updateProfile , updateEmail, sendEmailVerification, updatePassword , sendPasswordResetEmail, deleteUser    } from "firebase/auth";
...
const changeAdresseMail = () => {
    updateEmail(user as any, email.value).then(() => {
  // Email updated!
  // ...
...
const changepassWord = () => {
    updatePassword(user as any, newPassword.value).then(() => {
  // Update successful.
