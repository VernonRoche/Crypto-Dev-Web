function autoComplete(input:HTMLInputElement,completeValue:Array<string>):boolean{
    ...
    input.addEventListener('input', function(e){
      ...
      completeValue.forEach( (elem:string) => {
        if(elem.substring(0,val.length).toUpperCase() == val.toUpperCase()){
          ...
          itemDiv.innerHTML = "<strong>" + elem.substring(0,val.length) +"</strong>";
    ...
    input.addEventListener("keydown", function(e) {
      let x:HTMLDivElement = document.getElementById(this.id + "autocomplete-list");
      if(x){
    ...
    function closeAlllists(elem=null){
      let x:HTMLCollection = document.getElementsByClassName("autocomplete-items");
