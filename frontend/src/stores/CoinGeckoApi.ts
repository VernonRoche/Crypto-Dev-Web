export namespace CoinGeckoApi{
    ...
    */
     ...
     * @returns  Promise responce from the fetch
     */
    export async function getCryptoPrice(id:string, currency:string="eur", market_cap:boolean=false, hour_vol:boolean=false, hour_change:boolean=false, last_update:boolean=false, precision:number=3): Promise<JSON> {
        const URL  = "https://api.coingecko.com/api/v3/simple/price?ids="+id+"&vs_currencies="+currency+"&include_market_cap="+market_cap+"&include_24hr_vol="+hour_vol+"&include_24hr_change="+hour_change+"&include_last_updated_at="+last_update+"&precision="+precision;
        const responce = await fetch(URL);
        return await responce.json();
    }
     ...
     * @returns Promise responce from the fetch
     */
    export async function Search(query:string=""): Promise<JSON> {
