<template v-if="Login.isLog">
  <div class="inline">
    <div id="FavoriteList" class="container mx-auto aspect-auto"></div>
  </div>
</template>
...
export default {
  ...
  async mounted() {
    document
      .querySelectorAll("[id=Favrow]")
      .forEach((e: HTMLTableRowElement) => {
    ...
    if (user) {
      ...
      if (favorites.length > 0) {
        ...
        favorites[0].favorite.forEach((element) => {
          ...
          let favImg: HTMLCollectionOf<HTMLImageElement> =
            row.getElementsByClassName("FavIcon cursor-pointer");
